...
$stmt = $conn->prepare("SELECT customer_name, email, product_id, quantity, status, order_date FROM orders WHERE id = ?");
$stmt->bind_param("i", $order_id);
$stmt->execute();
$stmt->bind_result($customer_name, $email, $product_id, $quantity, $status, $order_date);
if (!$stmt->fetch()) {
    die("Order not found.");
}
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update if no errors
    if (!$errors) {
        // Get product name for new product_id
        $productStmt = $conn->prepare("SELECT name FROM products WHERE id = ?");
        $productStmt->bind_param("i", $new_product_id);
        $productStmt->execute();
        $productStmt->bind_result($product_name);
        $productStmt->fetch();
        $productStmt->close();

        $updateStmt = $conn->prepare("UPDATE orders SET customer_name=?, email=?, product_id=?, product_name=?, quantity=?, status=?, order_date=? WHERE id=?");
        $updateStmt->bind_param("ssisissi", $customer_name, $email, $new_product_id, $product_name, $quantity, $status, $order_date, $order_id);

        if ($updateStmt->execute()) {
            $success = true;
            // Refresh order_id variables with new values to show in form
...
<header>
    <h1>✏️ Edit Order #<?= htmlspecialchars($order_id) ?></h1>
</header>
...
<div class="container">
    ...
    <form method="post" action="edit_order.php?id=<?= $order_id ?>" novalidate>
        ...
        <select id="product_id" name="product_id" required>
            ...
            <?php
            // Reset result pointer and loop products
